...
Плата индикации должна обеспечивать выдачу звуковых сигналов в следующих случаях:
    Режимы BUZZER: (Задаются значением байта Buffer[2])
0. Выключен
1. Постоянный сигнал
...
#endif
    if (Buffer[2] == 48) //Выключен
    {
        PORT_ResetBits(MDR_PORTB, BUZZER);
    }
    else if (Buffer[2] == 49) //Постоянный сигнал
    {
        PORT_SetBits(MDR_PORTB, BUZZER);
    }
    else if (Buffer[2] == 50) //Два сигнала (звучание в течение 0,5с с интервалом 0,5с).
    {
        PORT_ResetBits(MDR_PORTB, BUZZER);
    }
    else if (Buffer[2] == 51) //Три сигнала (звучание в течение 0,5с с интервалом 0,5с).
    {

    }
    else if (Buffer[2] == 52) //Четыре сигнала (звучание в течение 0,5с с интервалом 0,5с).
    {

    }
    else if (Buffer[2] == 53) //Один сигнал (звучание в течение 0,5с с интервалом 60с).
    {

    }
    else if (Buffer[2] == 54) //Непрерывная последовательность (звучание в течение 0,5с с интервалом 0,5с).
    {
