...
/*************************************************************************************
  С помощью АЦП измеряется напряжение, снимаемое с внешнего потенциометра, 
	подключенного к каналу ADC6, и температура кристалла МК с использованием 
	встроенного датчика температуры. 
  Применяется режим многократного преобразования c автоматическим переключением 
	нескольких каналов и использованием прямого доступа к памяти. 	
...
__task void Main_Task_Init (void)
{
  ...
  U_ADC_Task_ID = os_tsk_create (U_ADC_Task_U_T_DMA_Function, 10);
